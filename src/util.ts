...
export function padStart(data: string, length: number): string[] {
    return [
        ...Array((length - (data.length % length)) % length).fill("0"),
        ...data
    ].reduce(getMapper(length), []);
}
...
export function padEnd(data: string, length: number): string[] {
    return [
        ...data,
        ...Array((length - (data.length % length)) % length).fill("=")
    ].reduce(getMapper(length), []);
}
...
export function quickMap(
    ...
    return Array(length)
        .fill(0)
        .map((_, x) => multiplier(reverse ? -x + length - 1 : x));
}
...
export function stringToNumber(str: string, charset: string) {
    return str
        .split("")
        .reverse()
        .map(x => charset.indexOf(x))
        .reduce((a, x, i) => a + (x >= 0 ? x * charset.length ** i : 0), 0);
}
